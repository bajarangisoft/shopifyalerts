$(function () {
    $("body").on('change', 'select', function () {
        ...
        if(next_selector == 'time_picker'){
            $('#time_picker select').each(function () {
                ...
                //     alert('filter');
                    // $(this).attr({ name: "filter_condition_"+data_unique+"[]"});
                    $(this).addClass("action");
                }
                $(this).attr({ name: "action_condition_"+data_unique+"[]"});
            ...
            if($(this).hasClass("action")){
                $('#'+next_selector).attr({ name: "action_condition_"+data_unique+"[]"});
                $('#'+next_selector).addClass("action");
            }else{
                $('#'+next_selector).attr({ name: "filter_condition_"+data_unique+"[]"});
            }
        }
        // var selectorHTML = $('.selector-stash').find('#' + next_selector).clone();
        var selectorHTML = $('.selector-stash').find('#'+next_selector).clone().prop('id', "cloned_'"+data_unique+"'");
        ...
        // $('#filter_condition_'+condition_block+addon_cond_count).val()
    });
    ...
    $('body').on('click', '.add-condition-block', function () {
        ...
        conditionrRowHTML += "<div class=\"condition-block-wrap\"  data-condition-block=\""+cond_count+"\">" +
            ...
            "                           <input type=\"text\" name=\"condition_blks_filter_count_"+cond_count+"\" id=\"condition_blks_filter_count_"+cond_count+"\" value=\"1\">\n" +
            "                            <input type=\"text\" name=\"condition_action_count_"+cond_count+"\" id=\"condition_action_count_"+cond_count+"\" value=\"1\">" +
            "                                <div class=\"px-6 py-4 \">\n" +
            ...
            "                                                        <span>\n" +
            "                                                        <select id=\"condition_choice_"+cond_count+"\" name=\"condition_choice_"+cond_count+"\" class=\" inline-grid mr-1.5 py-1.5 pr-8 text-sm shadow-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md \">\n" +
            ...
            "                                                <select id=\"action_type_"+cond_count+"_1\" data-condition=\""+cond_count+"\" name=\"action_type_"+cond_count+"_1\" class=\"action inline-grid m-1.5 py-1.5 pr-8 text-sm shadow-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md \">\n" +
            "                                                    <option selected=\"selected\" disabled=\"disabled\" value=\" \">\n" +
            "                                                        Action type\n" +
            "                                                    </option>\n" +
    ...
    $('body').on('click', '.add-action-line', function () {
        ...
        $('#condition_action_count_'+condition_block).val(action_count);
    });
});
